// vortex.js (r12): time-based motion, one-letter-per-cell, hard speed cap
class VortexText extends HTMLElement {
  connectedCallback() {
    ...
    this.speed    = num("data-speed", 0.8);           // base rad/s
    ...
    this.maxSpd   = Math.max(0, num("data-maxspd", 0.05));              // ABS cap rad/s
    this.debugHUD = !!(this.getAttribute("data-debug") ?? this.dataset.debug);

    // Back-compat for old per-frame speeds
    if (this.speed > 0 && this.speed < 0.2) this.speed *= 60;

    // Tight text (no spaces)
  ...
  _tick(now) {
    ...
    // inner→outer draw, one glyph per cell
    ...
    if (this.debugHUD) {
      ...
      ctx.fillText(`morph=${morph.toFixed(2)}  scale=${(speedScale*100).toFixed(3)}%`, 16, 30);
      ctx.fillText(`speedNow=${speedNow.toFixed(5)} rad/s`, 16, 46);
      ctx.restore();
    }
